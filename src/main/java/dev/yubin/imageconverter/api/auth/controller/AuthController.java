...
public class AuthController {
	...
	@Operation(summary = "Nest OAuth 콜백 수신", description = "Nest에서 받은 사용자 정보를 받아 JWT를 발급하고 사용자 " + "정보를 반환합니다.")
	@ApiResponses({
		@ApiResponse(responseCode = "200", description = "로그인 또는 회원가입 성공", content = @Content(schema = @Schema(implementation = TokenWithUserResponse.class))),
		@ApiResponse(responseCode = "400", description = "요청 형식 오류 (예: 필수 필드 누락)", content = @Content(schema = @Schema(implementation = ErrorResponse.class))),
		@ApiResponse(responseCode = "404", description = "유저를 찾을 수 없음", content = @Content(schema = @Schema(implementation = ErrorResponse.class))),
		@ApiResponse(responseCode = "500", description = "서버 내부 오류 (예: DB 오류, JWT 발급 실패)", content = @Content(schema = @Schema(implementation = ErrorResponse.class)))})
	public ResponseEntity<TokenWithUserResponse> receiveUserInfoFromNest(
		...
		String jwt = jwtProvider.generateToken(user.getPublicId(), user.getProvider());
