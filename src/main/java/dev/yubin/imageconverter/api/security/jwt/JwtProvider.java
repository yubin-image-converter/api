...
public class JwtProvider {
    ...
    public String generateToken(String publicId, OAuthProvider provider) {
        ...
        return Jwts.builder().setSubject(publicId).claim("provider", provider.name()).setIssuedAt(now).setExpiration(expiryDate).signWith(key, SignatureAlgorithm.HS256).compact();
    }
